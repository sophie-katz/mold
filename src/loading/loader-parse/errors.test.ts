...
describe('File count', () => {
  test('1 file', () => {
    ...
    expect(error.message).toBe('maximum file count limit exceeded: 1 file and/or directory');
  });

  test('2 files', () => {
    const error = new ErrorLoaderParseFileCount(2);
    expect(error.message).toBe('maximum file count limit exceeded: 2 files and/or directories');
  });
});
...
describe('Memory usage', () => {
  ...
  test('1 KB', () => {
    ...
    expect(error.message).toBe('maximum memory usage limit exceeded: 1024 bytes (0.001 MB)');
  });

  test('1023 KB', () => {
    const error = new ErrorLoaderParseMemoryUsage(1024 * 1023);
    expect(error.message).toBe('maximum memory usage limit exceeded: 1047552 bytes (0.999 MB)');
  });

  test('1 MB', () => {
    const error = new ErrorLoaderParseMemoryUsage(1024 * 1024);
    expect(error.message).toBe('maximum memory usage limit exceeded: 1048576 bytes (1.000 MB)');
  });

  test('128 MB', () => {
    const error = new ErrorLoaderParseMemoryUsage(1024 * 1024 * 128);
    expect(error.message).toBe('maximum memory usage limit exceeded: 134217728 bytes (128.000 MB)');
  });
});
